...
public class LazyAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            x = parent.getContext();
            convertView = LayoutInflater.from(x).inflate(R.layout.item_album,
                    parent, false);
            viewHolder = new ViewHolder();

            viewHolder.text = (TypefaceTextView) convertView
        ...
        imageLoader.DisplayImage(data.get(position).getPath2(),
                viewHolder.image, data.get(position).getSong(),
