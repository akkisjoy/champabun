/*
 * DragSortListView. A subclass of the Android ListView component that enables drag and drop re-ordering of list items. Copyright 2012 Carl Bauer
...
package champak.champabun.business.utilities.dragSortListView;
...
public class DragSortListView extends ListView {
    ...
    public DragSortListView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            if (useDefault) {
                ...
                //int bgColor = a.getColor(R.styleable.DragSortListView_float_background_color, getResources().getColor(R.color.transparentblack));
                ...
                controller.setBackgroundColor(getResources().getColor(R.color.transparentblack));
    ...
    private void drawDivider(int expPosition, Canvas canvas) {
        ...
        // Logger.d("mobeta", "div="+divider+" divH="+dividerHeight);
        ...
        if (divider != null && dividerHeight != 0) {
            ...
            if (expItem != null) {
                ...
                // Logger.d("mobeta", "l="+l+" t="+t+" r="+r+" b="+b);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mFloatView != null) {
            ...
            canvas.save();
            // Logger.d("mobeta", "clip rect bounds: " + canvas.getClipBounds());
    ...
    private void printPosData() {
        Logger.d("mobeta", "mSrcPos=" + mSrcPos + " mFirstExpPos=" + mFirstExpPos + " mSecondExpPos=" + mSecondExpPos);
    }
    ...
    private boolean updatePositions() {
        ...
        if (mAnimate) {
            ...
            // Logger.d("mobeta", "edgeTop="+edgeTop+" edgeBot="+edgeBottom);
    ...
    private void adjustOnReorder() {
        ...
        // Logger.d("mobeta", "first="+firstPos+" src="+mSrcPos);
        if (mSrcPos < firstPos) {
            ...
            // Logger.d("mobeta", "top="+top+" fvh="+mFloatViewHeight);
     ...
     *
     * @param remove Remove the dragged item from the list. Calls a registered RemoveListener, if one exists. Otherwise, calls the DropListener, if one
     *               exists.
    ...
    private int getChildHeight(int position) {
        ...
        if (v != null) {
            ...
            if (childHeight != -1) {
                // Logger.d("mobeta", "found child height in cache!");
                return childHeight;
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // Logger.d("mobeta", "onMeasure called");
        if (mFloatView != null) {
    ...
    protected boolean onDragTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_CANCEL:
                ...
                break;
            case MotionEvent.ACTION_UP:
                // Logger.d("mobeta", "calling stopDrag from onDragTouchEvent");
                if (mDragState == DRAGGING) {
                    stopDrag(false);
                }
                doActionUpOrCancel();
     ...
     * @param dragFlags Flags that restrict some movements of the floating View. For example, set <code>dragFlags |=
     *                  ~{@link #DRAG_NEG_X}</code> to allow dragging the floating View in all directions except off the screen to the left.
    ...
    private void doDragFloatView(int movePos, View moveItem, boolean forceInvalidate) {
        ...
        if (updated) {
            ...
            // Logger.d("mobeta", "  adjust scroll="+scroll);
    ...
    private void updateFloatView() {
        ...
        // Logger.d("mobeta", "dragView top=" + (y - mDragDeltaY));
        // Logger.d("mobeta", "limit=" + limit);
        // Logger.d("mobeta", "mDragDeltaY=" + mDragDeltaY);
     ...
     * determines how items are lifted (see {@link setDragInitMode(int)}). If disabled, items cannot be dragged.
     *
     ...
     * {@link BaseAdapter#notifyDataSetChanged()} or something like it is called in your implementation. Furthermore, if you have a choiceMode other
     * than none and the ListAdapter does not return true for {@link ListAdapter#hasStableIds()}, you will need to call
     * {@link #moveCheckState(int, int)} to move the check boxes along with the list items.
     *
     * @param l
     */
     ...
     * method must be called when the order of items changes in an underlying adapter which does not have stable IDs (see
     * {@link ListAdapter#hasStableIds()}). This is because without IDs, the ListView has no way of knowing which items have moved where, and cannot
     * update the check state accordingly.
     * <p/>
     * A word of warning about a "feature" in Android that you may run into when dealing with movable list items: for an adapter that <em>does</em>
     * have stable IDs, ListView will attempt to locate each item based on its ID and move the check state from the item's old position to the new
     * position — which is all fine and good (and removes the need for calling this function), except for the half-baked approach. Apparently to save
     * time in the naive algorithm used, ListView will only search for an ID in the close neighborhood of the old position. If the user moves an item
     * too far (specifically, more than 20 rows away), ListView will give up and just force the item to be unchecked. So if there is a reasonable
     * chance that the user will move items more than 20 rows away from the original position, you may wish to use an adapter with unstable IDs and
     ...
     * this method must be called when the order of items changes in an underlying adapter which does not have stable IDs (see
     * {@link ListAdapter#hasStableIds()}). This is because without IDs, the ListView has no way of knowing which items have moved where, and cannot
     * update the check state accordingly.
     * <p/>
    ...
    /**
     * Interface for customization of the floating View appearance and dragging behavior. Implement your own and pass it to
     * {@link #setFloatViewManager}. If your own is not passed, the default {@link SimpleFloatViewManager} implementation is used.
     */
    public interface FloatViewManager {
         ...
         *
         * @param position Position of item to drag (NOTE: <code>position</code> excludes header Views; thus, if you want to call
         *                 {@link ListView#getChildAt(int)}, you will need to add {@link ListView#getHeaderViewsCount()} to the index).
    ...
    private class DragScroller implements Runnable {
        ...
        public void run() {
            ...
            if (scrollDir == UP) {
                View v = getChildAt(0);
                // Logger.d("mobeta", "vtop="+v.getTop()+" padtop="+padTop);
                if (v == null) {
                    mScrolling = false;
                    return;
                } else {
            ...
            mPrevTime = mCurrTime;
            // Logger.d("mobeta", "  updated prevTime="+mPrevTime);
